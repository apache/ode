...
class AssignGenerator extends DefaultActivityGenerator {
	private static final Logger __log = LoggerFactory
			.getLogger(AssignGenerator.class);
	...
	private OAssign.RValue compileFrom(From from, Object requestedResultType) {
		...
		try {
			if (from.isExtensionVal()) {
				...
				OAssign.PropertyRef pref = new OAssign.PropertyRef(
						_context.getOProcess());
				...
				OAssign.VariableRef vref = new OAssign.VariableRef(
						_context.getOProcess());
				...
				OAssign.PartnerLinkRef plref = new OAssign.PartnerLinkRef(
						_context.getOProcess());
				...
				return new OAssign.Expression(_context.getOProcess(),
						_context.compileExpr(from.getAsExpression(), null,
		...
		try {
			if (to.isPropertyVal()) {
				...
				return new OAssign.LValueExpression(_context.getOProcess(),
						_context.compileLValueExpr(to.getAsExpression(), null,
								null, resultType));
			}
