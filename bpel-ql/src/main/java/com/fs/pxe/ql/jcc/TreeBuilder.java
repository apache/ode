...
import com.fs.pxe.ql.tree.nodes.Conjunction;
import com.fs.pxe.ql.tree.nodes.Disjunction;
...
public class TreeBuilder extends Builder<String> {
    ...
    public com.fs.pxe.ql.tree.nodes.Node build(String query) {
      try {
        com.fs.pxe.ql.jcc.ASTStart start = new Parser(query).start();
        return build(start);
    ...
    private com.fs.pxe.ql.tree.nodes.Node build(Node node) {
    ...
    private Identifier createIdentifier(Node parentNode, int index) {
        ...
        if(node instanceof ASTField) {
            return new com.fs.pxe.ql.tree.nodes.Field(((ASTField)node).name);
        }
        if(node instanceof ASTProperty) {
            return new com.fs.pxe.ql.tree.nodes.Property(((ASTProperty)node).getName());
        }
        //TODO
        throw new IllegalArgumentException("");
    }
