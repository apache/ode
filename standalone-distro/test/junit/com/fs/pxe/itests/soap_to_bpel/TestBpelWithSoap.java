 ...
 *
 */
package com.fs.pxe.itests.soap_to_bpel;
...
import com.fs.pxe.soap.SoapServiceConstants;
import com.fs.pxe.httpsoap.HttpSoapInteraction;
import com.fs.pxe.httpsoap.HttpSoapInteraction;
import com.fs.pxe.httpsoap.HttpSoapInteraction;
import com.fs.pxe.httpsoap.HttpSoapInteraction;
...
public class TestBpelWithSoap extends junit.framework.TestCase
  implements com.fs.pxe.bpel.util.deployment.ServiceProviderInfoCallback {
  ...
  private com.fs.pxe.sfwk.impl.DomainNodeImpl dNode;
   ...
   */
  public com.fs.pxe.bpel.util.deployment.ServiceProviderInfo getInfo(String procesName,
                                                                     String partner,
                                                                     String role,
                                                                     boolean isMyRole) {
    ...
    if (partner.startsWith("customer")) {
      return new com.fs.pxe.bpel.util.deployment.ServiceProviderInfo(_soapServiceUri,
                                                                     p);
    } else if (partner.equals("echoer")) {
      return new com.fs.pxe.bpel.util.deployment.ServiceProviderInfo(_echoServiceUri,
                                                                     p);
    } else {
  ...
  public void testReceiveCorrelation()
                              throws Exception {
    com.fs.pxe.sfwk.cci.ServiceProviderConnectionFactory spCF = com.fs.pxe.sfwk.cci.ServiceProviderConnectionFactory
                                                                .newInstance(com.fs.pxe.sfwk.cci.ServiceProviderConnectionFactory.BINDING_MEM);
    com.fs.pxe.sfwk.cci.ServiceProviderConnection conn = spCF.getConnection(new com.fs.pxe.sfwk.DomainUUID("DOMAIN:"
                                                                                                           + getClass().getName()),
                                                                            _soapServiceUri,
                                                                            null);
    ...
    for (int j = 0; j < 10; ++j) {
      ...
      com.fs.pxe.bpel.runtime.dao.mem.StateStoreConnectionFactoryImpl ssc = new com.fs.pxe.bpel.runtime.dao.mem.StateStoreConnectionFactoryImpl();
      String[] stores = ssc.getStateStores();
      assert stores.length == 1;

      com.fs.pxe.bpel.runtime.dao.mem.ProcessDaoImpl process = (com.fs.pxe.bpel.runtime.dao.mem.ProcessDaoImpl)ssc
                                                               .getProcess(stores[0],
                                                                           "loanApprovalProcess");
      ...
      while (true) {
        ...
        for (Iterator iter = process.getInstances()
          ...
          com.fs.pxe.bpel.runtime.dao.ProcessInstanceDAO instance = (com.fs.pxe.bpel.runtime.dao.ProcessInstanceDAO)iter
                                                                    .next();
          int state = instance.getState();

          if (state == com.fs.pxe.bpel.runtime.dao.ProcessInstanceDAO.STATE_COMPLETED) {
            completed++;
          } else if (state == com.fs.pxe.bpel.runtime.dao.ProcessInstanceDAO.STATE_ACTIVE) {
            active++;
          }
        }
  ...
  protected void setUp()
    ...
    com.fs.pxe.bpel.util.deployment.BpelServiceGenerator gen = com.fs.pxe.bpel.util.deployment.BpelServiceGenerator
                                                               .newGenerator(this);
    gen.generateSar(_sarFile, _bpel, _wsdl);

    Properties domainProps = new Properties();
    domainProps.put("pxe.config.domainUUID", "DOMAIN:" + getClass().getName());
    domainProps.put("pxe.config.DomainStateConnectionFactory",
                    com.fs.pxe.sfwk.impl.dao.mem.DeploymentStoreDConnectionFactoryImpl.class
                    .getName());

    domainProps.put("pxe.config.provider.sp1.uri", _soapServiceUri);
    domainProps.put("pxe.config.provider.sp1.class",
                    com.fs.pxe.adapters.httpsoap.HttpSoapAdapterInbound.class.getName());

    domainProps.put("pxe.config.provider.sp2.uri", _bpelServiceUri);
    domainProps.put("pxe.config.provider.sp2.class",
                    com.fs.pxe.bpel.provider.BpelServiceProvider.class.getName());

    domainProps.put("pxe.config.provider.sp3.uri", _echoServiceUri);
    domainProps.put("pxe.config.provider.sp3.class",
                    com.fs.pxe.soap.echo.EchoClientProvider.class.getName());

    com.fs.pxe.sfwk.impl.config.DomainConfig cfg = new com.fs.pxe.sfwk.impl.config.DomainConfig(domainProps,
                                                                                                getClass().getClassLoader());
    com.fs.pxe.sfwk.bindings.mem.MemDomainNode memDomainNode = new com.fs.pxe.sfwk.bindings.mem.MemDomainNode(cfg);
    dNode = memDomainNode.getDomainNode();

    com.fs.pxe.sfwk.mngmt.ISystem system = dNode.getIDomain()
                                                .deploySystem(new com.fs.pxe.sfwk.deployment.SarDeploymentBundle(_sarFile));
    system.activate();
    dNode.bindAll();
  }
  ...
  private javax.xml.soap.SOAPMessage makeMsg(String name, int amount,
    ...
    String s = com.fs.utils.DOMUtils.domToString(doc.getDocumentElement());
