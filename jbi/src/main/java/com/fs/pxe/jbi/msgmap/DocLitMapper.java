...
public class DocLitMapper extends BaseXmlMapper implements Mapper {
  ...
  public void toNMS(NormalizedMessage nmsMsg,
      com.fs.pxe.bpel.iapi.Message pxeMsg, Message msgdef)
    ...
    Element part = DOMUtils.findChildByName(pxeMsg.getMessage(),new QName(null, partdef.getName()));
    if (part == null) {
      String errmsg = "PXE message did not contain expected part: " +  partdef.getName();
      __log.debug(errmsg);
    ...
    if (content == null) {
      String errmsg = "PXE message did not contain element " + partdef.getElementName() + " in part: " +  partdef.getName();
      __log.debug(errmsg);
      throw new MessageTranslationException(errmsg);
    }
  ...
  public void toPXE(com.fs.pxe.bpel.iapi.Message pxeMsg,
