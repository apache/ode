 ...
 */
class PxeServiceUnit {
  private static final Log __log = LogFactory.getLog(PxeServiceUnit.class);
  ...
  /** Ctor. */
  PxeServiceUnit(PxeContext pxe, String serviceUnitID, String serviceUnitRootPath) {
    _pxe = pxe;
  ...
  public void deploy() throws DeploymentException {
    boolean abortOnFailure = !_pxe._config.getAllowIncompleteDeployment();
    ...
    for (File dd : _descriptors) {
      try {
        ...
        _pxe._server.deploy(pid, dd.toURI());
        deployed.add(pid);
      } catch (Exception ex) {
        __log
            .error(__msgs.msgPxeProcessDeploymentFailed(dd, _serviceUnitID), ex);
        ex1 = ex;
        if (abortOnFailure)
          break;
      }
  ...
  public void undeploy() throws Exception {
    ...
    for (QName x : _pids)
      try {
        boolean undeployed = _pxe._server.undeploy(x);
        if (undeployed)
          __log.debug("undeployed pxe process: " + x);
        else
          __log.debug("skipped undeployment (not deployed): " +x);
      } catch (Exception ex) {
        String errmsg = __msgs.msgPxeProcessUndeploymentFailed(x);
  ...
  public void start() throws Exception {
    ...
    if (activated.size() != _pids.size()) {
      for (QName pid : activated)
        try {
          _pxe._server.deactivate(pid,true);
        } catch (Exception ex) {
  ...
  private QName genPid(String serviceUnitID, String ddName) {
    return new QName(_pxe._config.getPidNamespace(), serviceUnitID + "/"
        + ddName);
  }
