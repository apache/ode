...
/**
 * Server class called by our Axis hooks to handle all PXE lifecycle
 * management.
 */
public class PXEServer {
  ...
  private PXEConfigProperties _pxeConfig;
  ...
  public void init(ServletConfig config, AxisConfiguration axisConf)  throws ServletException {
    ...
    _pxeConfig = new PXEConfigProperties(_appRoot);
    _pxeConfig.load();
  ...
  private void initExternalDb() throws ServletException {
    try {
      _datasource = lookupInJndi(_pxeConfig.getDbDataSource());
      __log.info(__msgs.msgPxeUsingExternalDb(_pxeConfig.getDbDataSource()));
    } catch (Exception ex) {
      String msg = __msgs.msgPxeInitExternalDbFailed(_pxeConfig.getDbDataSource());
      __log.error(msg,ex);
  ...
  private void initEmbeddedDb() throws ServletException {
    ...
    String url =
            "jdbc:derby:" + _appRoot + "/" + _pxeConfig.getDbEmbeddedName();
    ...
    minervaPool.getPoolParams().maxSize = _pxeConfig.getPoolMaxSize();
    minervaPool.getPoolParams().minSize = _pxeConfig.getPoolMinSize();
