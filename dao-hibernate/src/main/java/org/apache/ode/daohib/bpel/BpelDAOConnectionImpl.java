...
class BpelDAOConnectionImpl implements BpelDAOConnection {
    ...
    static Iterator<HProcess> _processQuery(Session session, ProcessFilter filter) {
        ...
        if (filter.getNameFilter() != null) exampleProcess.setTypeName(filter.getNameFilter().replaceAll("\\*", "%"));
        if (filter.getNamespaceFilter() != null) exampleProcess.setTypeNamespace(filter.getNamespaceFilter().replaceAll("\\*", "%"));
